			...
			if (!reportedNodes.has(nodeKey)) {
				...
				context.report({
					...
					fix: (fixer) => {
						...
						const leadingComments = comments.filter((comment) => comment.loc.start.line === 1);

						if (leadingComments.length > 0) {
							// If there are leading comments, add the directive after them
							const lastLeadingComment = leadingComments[leadingComments.length - 1];
							return fixer.insertTextAfter(lastLeadingComment, "\n'use client';");
						} else {
